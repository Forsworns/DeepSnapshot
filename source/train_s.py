...
def train(label, phi, t_label, t_phi, cfg):
    ...
    for ep in range(cfg.epoch):
        ...
        for ep_i, batch in enumerate(data_loader):
            ...
            loss = loss_func(net_output*mask, initial*mask)
            loss.backward()
        ...
        with torch.no_grad():
            ...
            net_input, mask = masker.mask(
                v_initial, (ep_i+1) % (masker.n_masks - 1))
            net_input = net_input.to(cfg.device)
            net_output = model(net_input, v_y, phi)
            val_loss = loss_func(net_output*mask, v_initial*mask)
            ...
            print("ep_i ", ep, "loss ", round(loss.item(), 5), "val loss ",
                  round(val_loss, 5), "real val loss ", round(real_val_loss, 5), "lr", optimizer.param_groups[0]['lr'], "time ", time())

            if val_loss < best_val_loss:
                best_val_loss = val_loss
                best_img = np.clip(
                    net_output.detach().cpu().numpy(), 0, 1).astype(np.float64)
                best_psnr = compare_psnr(v_label.numpy(), best_img)
